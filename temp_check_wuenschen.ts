...
async function main() {
  const { data } = await supabase.from('german_verbs').select('infinitive, object_case, preposition, preposition_case, transitivity').eq('infinitive', 'sich wünschen').single();
  console.log(data);
}
main();