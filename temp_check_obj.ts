...
async function main() {
  const { data, error } = await supabase
    .from('german_verbs')
    .select('infinitive, object_case, preposition, preposition_case, transitivity')
    .eq('is_reflexive', true)
    .order('infinitive');
  
  console.log('Error:', error);
  for (const v of data) {
    console.log(`${v.infinitive}: objCase=${v.object_case}, prep=${v.preposition || 'none'} + ${v.preposition_case || ''}, trans=${v.transitivity}`);
  }
}
main();