...
async function main() {
  const { data, error } = await supabase.from('german_verbs').select('infinitive, is_reflexive, reflexive_pronoun, verb_type, separable_prefix, object_case, transitivity, preposition, preposition_case, auxiliary, partizip_2, slug').eq('is_reflexive', true);
